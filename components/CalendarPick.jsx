...
const CalendarPick = () => {
  ...
  const renderEventItem = ({ item, index, date }) => {
    return (
      <View className="bg-cardB border-2 min-w-full mb-5 border-borderB ">
        <Text className="text-white px-2.5 py-1 ">
          {item.name} - {item.time}
        </Text>
  ...
  const renderAllEvents = () => {
    ...
    if (allDates.length === 0) {
      return (
        <Text className="text-white  bg-cardB    space-y-2 mt-7   px-4 ">
          No bookings
        </Text>
      );
    }
      ...
      <View className="w-60 h-50  px-2.5 py-1 mx-0.5 my-0.5 ">
        {allDates.map((date) => (
          <View key={date} className="min-w-full">
            <Text className="bg-cardB min-w-full text-white rounded-t-lg px-2.5 py-1 border-x-2 border-t-2 border-borderB ">
              {date}
            </Text>
            <FlatList
              data={events[date]}
              renderItem={({ item, index }) =>
                renderEventItem({ item, index, date })
              }
